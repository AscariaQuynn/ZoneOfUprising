...
public class DamageControl extends ControlAdapter implements PhysicsCollisionListener {
    ...
    private ParticleController fracturedExplosion;
    ...
    /**
     * Sets particle controller that is shown while ship explodes and should show its fractured parts.
     * @param fracturedExplosion
     */
    public void setFracturedExplosion(ParticleController fracturedExplosion) {
        this.fracturedExplosion = fracturedExplosion;
    }
    ...
    public void explode() {
        // Show explosion
        if(null != fracturedExplosion) {
            worldManager.showEffect(fracturedExplosion);
        }
    ...
    private void showTestEmitter(Node node) {
        if(shown || !(node.getChild("Geometry") instanceof Geometry)) {
            return;
        }
        shown = true;

        Geometry geometry = (Geometry)node.getChild("Geometry");
        for(ParticleController controller : ParticleControllerHelper.getTest(app.getAssetManager(), geometry)) {
            worldManager.addToWorld("test", controller.getGeometry(), new SpawnPoint(node));
        }
    }
}
